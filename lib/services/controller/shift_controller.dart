...
import '../../views/pages/largePages/saleHistory/printing_history_view.dart';
  ...
  Future<void> shiftDetailsRequest() async {
    ...
    if (response.statusCode == 200) {
      ...
      jsonObject['data']['history'].forEach((element) {
        Get.find<CashController>().cashHistoryList.value.add(CashHistoryModel(data: element));
      });
            ...
            children: [
              ...
              pw.SizedBox(
                  ...
                  child: pw.ListView.separated(
                    itemCount: Get.find<CashController>().cashHistoryList.value.length, itemBuilder: (pw.Context context, int index) {
                    var currentItem = Get.find<CashController>().cashHistoryList.value[index];
                    return pw.Row(
                        mainAxisAlignment: pw.MainAxisAlignment.start,
                        children: [
                          pw.Expanded(flex: 3,child: pw.Column(
                              ...
                              children:[
                                pw.Text(currentItem.type.toString(),overflow: pw.TextOverflow.clip,),
                                pw.Text(currentItem.description.toString(),),
                              ])),
                          pw.Expanded(flex: 1,child: pw.Text(currentItem.amount.toString())),
                        ]);
